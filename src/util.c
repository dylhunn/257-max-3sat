...
int read_input(const char *file_path, formula *f) {
	...
	if ((fp = fopen(file_path, "r")) == 0) {
		printf("(input_file_not_readable) ");
	...
	*f = (formula) {.num_vars = -1, .num_clauses = -1, .clauses = 0};
	...
	while ((getline(&line, &line_length, fp)) != -1) { // read each line in the file
		...
		if (tok[0] == 'p') { // problem definition line
			if ((tok = strtok(NULL, " ")) == 0 ||
				strncmp(tok, "cnf", 3) != 0 || // bad file type token
			    (tok = strtok(NULL, " ")) == 0) { // missing data
					printf("(malformed_input_file) ");
					return -1;
			} 
			f->num_vars = atoi(tok);
			if ((tok = strtok(NULL, " ")) == 0) {
		...
		if (tok != 0) {
			printf("(malformed_input_file) ");
			return -4; // extra variables in clause
		}
		curr_clause++;
		if (curr_clause > f->num_clauses) {
			printf("(malformed_input_file) ");
			return -4; // extra clauses in file
		}
    }
    free(line);
	fclose(fp);
